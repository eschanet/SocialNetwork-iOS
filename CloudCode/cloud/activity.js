...
Parse.Cloud.afterSave('Activity', function(request) {
  console.log('Push Cloud afterSave begins');

  // Only send push notifications for new activities
  if (request.object.existed()) {
    console.log('Object existed before. Abort.');
    return;
  }
  ...
  if (request.object.get("type") === "comment" || request.object.get("type") === "comment-post" || request.object.get("type") === "comment-video" ) {   
      console.log('Push for a comment');
 ...
 if (request.object.get("type") === "mention") {
    ...
    console.log('Other query to ' + toUser.get('displayName') + ' with ID: ' + toUser.id);

  	var query = new Parse.Query(Parse.User);
  	
  	query.equalTo('username', toUser.get('username'));
  	
  	queryPush.matchesQuery("user", query);
